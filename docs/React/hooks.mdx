...
Some components need to synchronize with **external systems** outside of React (like a network or a third-party library). For example, you might want to set up a server connection, send an analytics log when a component appears on the screen, or control an external component based on a React state. _Effects_ let you run some code **after rendering** to do those kinds of things.
...
"Effects" refers to side effects caused by _rendering itself_, rather than by a _particular event_, and therefore **can't** be handled by event handlers.  
For example, sending a message in the chat is an _Event_ because it is directly caused by the user clicking a button. However, setting up a server connection is an _Effect_ because it needs to happen regardless what makes the component to appear.
...
Custom Hooks let you share _stateful logic_ but NOT _state_ itself.  
