...
**Effects** let you run some code **after rendering** to synchronize React components with **external systems** outside of React. For example, you might want to set up a server connection, send an analytics log when a component appears on the screen, or control an external element based on a React state.
...
Because `useEffectCode` runs after a component rerender, anything you use in your effect callback that **won't** trigger a re-render when updated should **NOT** go into the dependency array, e.g. refs from `useRef`
