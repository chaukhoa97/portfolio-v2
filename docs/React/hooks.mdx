...
```
  ...
  <TabItem value="setInterval" label="Using setInterval for time state" >
...
function useInterval() {
  ...
  useEffect(() => {
    ...
    const timeInterval = setInterval(() => setTime(new Date().getTime()), 1000)
...
```
  ...
  <TabItem value="run twice" label="Handle the Effect firing twice in dev" >
...
useEffect(() => {
  ...
  async function getData() {
    ...
    // 2. Here `ignore` is already set to `true` from the cleanup function, so `setTodos` won't be called,...
    // 2b. which is good as the `setTodos` will run on the 2nd time Effect run in dev
    // 4. But this time because the cleanup function is not called, here `ignore` is still `false`, so `setTodos` will be called
    if (!ignore) {
      setTodos(json)
    }
  }

  // 3. On the 2nd time Effect run in dev, `getData` is called again...
  getData()

  // 1. Cleanup function doesn't wait for async fn (getData) to finish...
  return () => {
    ignore = true
  }
}, [userId])
...
- Example: Focus on an input or trigger an animation when it appears on the screen.

```jsx title="useViewport.js"
import { useState, useEffect, useRef } from 'react'

export const useViewport = (isOneTime = true) => {
...
function Counter() {
  ...
  const handleClick = () => {
    // Callback from: Update dựa theo prev state, convention là letter đầu tiên (count -> c)
    setCount((c) => c + 5)
