...
_Effects_ let you run some code after rendering because some components need to synchronize with external systems outside of React (like network or a third-party library). For example, you might want to set up a server connection, send an analytics log when a component appears on the screen, or control an external component based on a React state.
