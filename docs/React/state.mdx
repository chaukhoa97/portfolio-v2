...
export const CartProvider = ({ children }) => {
  ...
  return (
    <CartContext.Provider value={memoizedValue}>
      // Use `children` so that the whole tree below `children` doesn't render
      all the time when you update the context value // Read more "Lift ur
      component up and pass it down as a prop"
      {children}
...
```jsx
// This will not re-render as expected because the `todos`'s reference hasn’t changed
...
| --------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------- |
| [Adding](https://beta.reactjs.org/learn/updating-arrays-in-state#adding-to-an-array)                | `push`, `unshift`                   | `concat`, `[...arr1, ...arr2]` (spread syntax)                                           |
...
| [Sorting](https://beta.reactjs.org/learn/updating-arrays-in-state#making-other-changes-to-an-array) | `sort`, `reverse`                   | Copy the array by spread syntax first: `cons nextList = [...list] -> nextList.reverse()` |
