...
// Init context with `null` is a must to check if `useCart` is used within a `CartProvider` (see `useCart` below)
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Memoizing the value which is passed as prop to `Provider` will reduce unnecessary re-render if it's NOT a top-level component
  // Being at the top-level means it has no other parent components that can trigger re-rendering to it...
  // It only gets re-rendered when `setCurrency` is called from its children to change the `currency`, so `useMemo` do nothing here
