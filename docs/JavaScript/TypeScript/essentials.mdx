...
// `never` use case with Conditional Types
...
// (Uncommon) Distributive Conditional Types
...
Assert the type-system that `x` can be assigned to `number`. Only be used to more or less specific type; không thể có ~ chuyển đổi vô lý như `"hello" as number`
