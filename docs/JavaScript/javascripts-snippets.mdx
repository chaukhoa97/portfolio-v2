...
```js
const eventDate = new Date() // Mon Nov 15 2021 09:37:35 GMT+0700 (GMT+07:00)
eventDate.toLocaleString() // 11/15/2021, 9:37:35 AM
eventDate.toLocaleDateString() // 11/15/2021
eventDate.toLocaleTimeString() // 9:37:35 AM
eventDate.getHours() // getMinutes(), getSeconds(), getMilliseconds()

const day = eventDate.getDay() // 1 (0 is Sunday, 1 is Monday, ...)
const date = eventDate.getDate() // 15
const month = eventDate.getMonth() // 10 (0 is January, 1 is February, ...)
const year = eventDate.getFullYear() // 2021. `getYear()` is deprecated

const eventDateInMs = eventDate.getTime() // 1636943855000. Useful for comparing date values. `Date.now()` is the same.
...
const eventIsoString = eventDate.toISOString()
const eventIsoDateString = eventDate.toISOString().split('T')[0]
const eventIsoTimeString = eventDate.toISOString().split('T')[1].slice(0, -1)
```
