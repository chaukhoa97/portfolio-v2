...
`this` refers to the object that is calling it. An exception is for arrow function, as it refers to its enclosing scope at the time the function is created.
...
const person = {
  ...
  test() {
    ...
    // `this` of a regular fn will be the JS `global` object
    this.foo.forEach(function (item) {
      console.log(this) // `window` (browser) or `global` (node)
    })

    // the 2nd arg of `forEach` can be used as the `this` of the callback fn, in this case, `person` obj
    this.foo.forEach(function (item) {
      console.log(this) // `person` obj
    }, this)

    // `this` of a ARROW fn will be the OBJECT that contains the method
