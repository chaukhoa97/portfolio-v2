...
`this` refers to **the object that is calling it**. An exception is for _arrow function_, as it refers to its **enclosing scope** at the time the function is created.
...
const person = {
  ...
  fn1() {
    ...
    console.log(this.foo) // [0, 1]
  },
  // `this` of the arrow fn `() => {...}` refers to its enclosing scope,
  // in this case is the JS global object -> `window` (browser) or `global` (node)
  fn2: () => {
  ...
  fn3() {
    ...
    this.foo.forEach(function (item) {
      console.log(this) // `person` obj
    }, this) // the 2nd arg of `forEach` can be used as the `this` of the callback fn, in this case is `person` obj

    // `this` of the arrow callback fn `(item) => {...}` refers to its enclosing scope,
    // in this case is `fn3` -> `person` obj
...
// `call` takes the `this` context as the 1st argument and the rest of the arguments are passed to the function
