...
- `type` ko declare thêm vào dc, còn `interface` thì có &rarr; dùng `interface` cho public API để người dùng tự thêm vào.
- Performance của `interface` cao hơn `type`, especially between intersections (`&` vs `extends`)
