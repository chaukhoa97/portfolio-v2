...
module.exports = {
  theme: {
    ...
    // Replaces all of the default `colors` values
    colors: {
      primary: {
        500: '#F47920',
        900: '#FEE6D3',
...
`extend`: Add/ replace new classes beside Tailwind's existing ones. If not using `extend`
all default configuration for that key will be overwritten.
